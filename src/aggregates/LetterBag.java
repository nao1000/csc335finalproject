...
public class LetterBag {
	/**
	 * This class represents a LetterBag which is an ArrayList of Letters available
	 * during the game.
	 * 
	...
	public void fillBag() {
		/**
		 * This method refills the letter bag with all of the letters used in a complete
		 * game of Scrabble
		 */
		letterBag.clear();
		for (int i = 0; i < 98; i++) {
	...
	public Letter draw(int letterI) {
		/**
		 * This method randomly selects (random because the index passed in should be
		 * random) a Letter for the player.
		 * 
		 * @param (int) letterI: a random index value provided via the model to select a
		 *              Letter
		 * 
	...
	public void addTo(Letter l) {
		/**
		 * This method adds a letter back (after a discard) to the LetterBag to keep it
		 * in play
		 * 
		 * @param (Letter) l : Letter object to add to bag
		 */
		letterBag.add(l);
	}
}
